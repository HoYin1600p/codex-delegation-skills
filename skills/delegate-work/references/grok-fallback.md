...
Codex owns scope, planning, usage decisions, validation and final acceptance. Use the bundled `scripts/grok_bridge.py` adapter and resolve `$SkillRoot` and `$Python` using [setup](setup.md); Grok Bot is unrelated. See the [shared routing policy](routing-policy.md) for fallback authorization. A failure other than confirmed Claude five-hour exhaustion does not automatically select Grok.
...
2. Run `& $Python -B "$SkillRoot/scripts/grok_bridge.py" preflight`. Stop on ambiguous account routing, missing executable or signed-out state. If needed, ask the user to run their official Grok CLI login. Never substitute an API key or switch billing.
...
4. Launch once: `& $Python -B "$SkillRoot/scripts/grok_bridge.py" run --task "<task.json>" [--handoff "<handoff-directory>"]`. Wait on the same process; silence never justifies a duplicate. One writing worker at a time.
...
`& $Python -B "$SkillRoot/scripts/grok_bridge.py" continue --task "<task.json>" --artifact "<artifact>" --grant-turns <N>`
...
`& $Python -B "$SkillRoot/scripts/grok_bridge.py" revise --task "<task.json>" --artifact "<artifact>" --feedback "<feedback.json>" --grant-turns <N>`
